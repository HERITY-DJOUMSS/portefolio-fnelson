/* ============================================================
   FN_SEC portfolio — script.js v4.0 CLEAN
   ============================================================ */
...
var DEFAULTS = {
  password: "Admin@2025",
  n8n_webhook: "",
  profile: { photo: null, name: "DJOUMESSI TATSIFANG", firstname: "Fredy Nelson", title: "Ingénieur Cybersécurité", location: "France", accroche: "Passionné par la cybersécurité et la protection des systèmes. Orienté Cloud et No-Code." },
  apropos: { paragraphes: ["Bonjour, je suis <strong>Fredy Nelson DJOUMESSI TATSIFANG</strong>, étudiant en ingénierie à l'ESAIP, spécialisé en cybersécurité.", "Mon objectif est de contribuer à la sécurisation des infrastructures numériques.", "Passionné par l'automatisation No-Code avec n8n, Make et les architectures Cloud."], photos: [] },
  infos: { nom: "Fredy Nelson DJOUMESSI", email: "fndjoumessi@gmail.com", tel: "+33 7 58 59 62 75", location: "France", formation: "ESAIP école d'ingénieur", specialite: "Cybersécurité" },
  competences: [
  ...
  certifications: [{ name: "TryHackMe", icon: "fas fa-trophy" }, { name: "HackTheBox", icon: "fas fa-box" }, { name: "[Certif]", icon: "fas fa-certificate" }],
...
(function() {
  ...
  var fs = 13;
  var drops = [];
  function initD() { drops = []; for (var i = 0; i < Math.floor(canvas.width / fs); i++) drops.push(1); }
  initD();
...
(function() {
  ...
  if (!el) return;
  var phrases = ['Ingénieur Cybersécurité', 'Expert No-Code & n8n', 'Cloud Security', 'CTF Player & Pentester'];
...
(function() {
  ...
  stars.forEach(function(s) {
    ...
    s.addEventListener('click', function() { var v = +s.dataset.val; if (ni) ni.value = v; stars.forEach(function(x) { x.classList.toggle('active', +x.dataset.val <= v); }); if (fb) fb.textContent = '✓ Note : ' + v + '/5'; });
  });
...
(function() {
  ...
  form.addEventListener('submit', function(e) {
    ...
    var ni = document.getElementById('noteInput');
    var payload = { nom: form.nom.value, email: form.email ? form.email.value : '', message: form.message.value, note: ni ? ni.value : '0', date: new Date().toLocaleString('fr-FR') };
    fetch(d.n8n_webhook, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload), mode: 'no-cors' })
...
/* ── PROJECT MODAL ── */
var currentSteps = [], currentStepIdx = 0;

function openProjectModal(projId) {
  var d = getData();
  var proj = null;
  for (var i = 0; i < d.projects.length; i++) { if (d.projects[i].id === projId) { proj = d.projects[i]; break; } }
  if (!proj) return;
  ...
  var cover = g('projModalCover');
  if (cover) cover.innerHTML = proj.cover ? '<img src="' + proj.cover + '" style="width:100%;height:100%;object-fit:cover"/>' : '<div style="display:flex;align-items:center;justify-content:center;height:100%;font-size:3rem;color:var(--green3)"><i class="' + (proj.icon || 'fas fa-code') + '"></i></div>';
  var links = g('projModalLinks');
  if (links) { var html = ''; if (proj.github) html += '<a href="' + proj.github + '" target="_blank" class="proj-link"><i class="fab fa-github"></i> Code</a>'; if (proj.demo) html += '<a href="' + proj.demo + '" target="_blank" class="proj-link proj-link-cyan"><i class="fas fa-external-link-alt"></i> Demo</a>'; links.innerHTML = html; }
  currentSteps = proj.steps || [];
  currentStepIdx = 0;
  var ss = g('stepsSection');
  if (ss) ss.style.display = currentSteps.length ? 'block' : 'none';
  if (currentSteps.length) renderStep();
  var overlay = g('projModalOverlay');
...
function renderStep() {
  ...
  var s = currentSteps[currentStepIdx];
  var ctr = document.getElementById('stepCounter'); if (ctr) ctr.textContent = (currentStepIdx + 1) + ' / ' + currentSteps.length;
  var disp = document.getElementById('stepDisplay');
  if (disp) disp.innerHTML = (s.image ? '<div class="step-image-wrap"><img src="' + s.image + '" class="step-image"/></div>' : '') + '<div class="step-content">' + (s.title ? '<div class="step-title"><i class="fas fa-chevron-right"></i> ' + s.title + '</div>' : '') + '<div class="step-caption">' + (s.caption || '') + '</div></div>';
  var dots = document.getElementById('stepsDots');
  if (dots) dots.innerHTML = currentSteps.map(function(_, i) { return '<div class="step-dot ' + (i === currentStepIdx ? 'active' : '') + '" onclick="goToStep(' + i + ')"></div>'; }).join('');
  var prev = document.getElementById('stepPrev'); if (prev) prev.disabled = currentStepIdx === 0;
  var next = document.getElementById('stepNext'); if (next) next.disabled = currentStepIdx === currentSteps.length - 1;
}

function goToStep(i) { currentStepIdx = i; renderStep(); }
...
function renderHero(d) {
  ...
  if (vc) {
    try {
      var cnt = parseInt(localStorage.getItem('fn_visit_count') || '0');
      if (!sessionStorage.getItem('fn_visited')) { cnt++; localStorage.setItem('fn_visit_count', cnt); sessionStorage.setItem('fn_visited', '1'); }
      vc.textContent = cnt;
...
function renderCompetences(d) {
  ...
  if (c) c.innerHTML = (d.competences || []).map(function(cat) {
    return '<div class="skill-category"><div class="skill-cat-title"><i class="' + cat.icon + '"></i> ' + cat.category + '</div><div class="skill-list">' +
      (cat.skills || []).map(function(s) { return '<div class="skill-item"><div class="skill-info"><span>' + s.name + '</span><span class="skill-pct">' + s.level + '%</span></div><div class="skill-bar"><div class="skill-fill" style="--w:' + s.level + '%"></div></div></div>'; }).join('') + '</div></div>';
  }).join('');
...
function renderExperiences(d) {
  ...
  var noMsg = function(icon, txt) { return '<div class="no-content-msg"><i class="' + icon + '"></i><p>' + txt + '</p></div>'; };
  var tl = function(e, badge, badgeClass) {
    return '<div class="tl-item"><div class="tl-date">' + (e.date || '') + '</div><div class="tl-card"><div class="tl-card-top"><div class="tl-title">' + (e.title || '') + '</div><div class="tl-badge ' + (badgeClass || '') + '">' + badge + '</div></div>' +
      '<div class="tl-company"><i class="fas fa-building"></i> ' + (e.company || e.institution || '') + ' · ' + (e.location || '') + '</div><div class="tl-desc">' + (e.description || '') + '</div>' +
      (e.tags && e.tags.length ? '<div class="tl-tags">' + e.tags.map(function(t) { return '<span class="tl-tag">' + t + '</span>'; }).join('') + '</div>' : '') + '</div></div>';
  };
  var st = document.getElementById('stages-timeline');
  if (st) st.innerHTML = exp.stages && exp.stages.length ? exp.stages.map(function(e) { return tl(e, 'STAGE', ''); }).join('') : noMsg('fas fa-briefcase', 'Ajoute tes stages depuis l\'interface admin.');
  var at = document.getElementById('academique-timeline');
  if (at) at.innerHTML = exp.academique && exp.academique.length ? exp.academique.map(function(e) { return tl(e, 'DIPLÔME', 'tl-badge-cyan'); }).join('') : noMsg('fas fa-graduation-cap', 'Ajoute ta formation depuis l\'interface admin.');
  initExpTabs();
}
...
function renderProjects(d) {
  ...
  if (!d.projects || !d.projects.length) { grid.innerHTML = '<div class="no-content-msg"><i class="fas fa-folder-open"></i><p>Aucun projet. Connecte-toi à l\'interface admin pour en ajouter.</p></div>'; return; }
  grid.innerHTML = d.projects.map(function(proj, i) {
...
function renderInterets(d) {
  ...
  if (g) g.innerHTML = (d.interets || []).map(function(it) {
    return '<div class="interet-card"><div class="interet-icon"><i class="' + it.icon + '"></i></div><div class="interet-title">' + it.title + '</div><div class="interet-desc">' + it.desc + '</div></div>';
  }).join('');
}
...
function renderAll() {
  ...
  try { renderCompetences(d); } catch(e) { console.error('Competences:', e); }
  try { renderExperiences(d); } catch(e) { console.error('Experiences:', e); }
  try { renderProjects(d); } catch(e) { console.error('Projects:', e); }
  try { renderInterets(d); } catch(e) { console.error('Interets:', e); }
}
...
// Refresh quand on revient sur la page depuis l'admin
...
window.addEventListener('storage', function(e) { if (e.key === 'fn_portfolio_data') renderAll(); });
