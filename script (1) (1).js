...
function renderApropos(d) {
  ...
  textContainer.innerHTML = paras.map(p => '<p class="apropos-p">' + p + '</p>').join('');

  const photosContainer = document.getElementById('apropos-photos-container');
  if (photosContainer && ap.photos && ap.photos.length > 0) {
    photosContainer.innerHTML = ap.photos.map(ph =>
      '<div class="apropos-photo-item reveal"><img src="' + ph.src + '" alt="' + (ph.caption||'') + '" class="apropos-photo"/><div class="apropos-photo-caption">' + (ph.caption||'') + '</div></div>'
    ).join('');
  }
...
function renderCompetences(d) {
  ...
  container.innerHTML = competences.map(cat => `
...
function renderAll() {
  ...
  setTimeout(() => { try { observeReveal(); addTilt(); } catch(e) {} }, 150);
}
...
// ── SYNC CLOUD ──
try { syncFromCloud(); } catch(e) { console.warn('syncFromCloud error:', e); }

// ── REFRESH automatique quand on revient sur la page depuis l'admin ──
document.addEventListener('visibilitychange', () => {
  if (document.visibilityState === 'visible') { try { renderAll(); } catch(e) {} }
});
window.addEventListener('focus', () => { try { renderAll(); } catch(e) {} });

// Refresh toutes les 3s si la page est visible (filet de securite)
setInterval(() => {
  if (document.visibilityState === 'visible') { try { renderAll(); } catch(e) {} }
}, 3000);

// ── STORAGE EVENT (autre onglet meme navigateur) ──
window.addEventListener('storage', (e) => {
  if (e.key === 'fn_portfolio_data') { try { renderAll(); } catch(e) {} }
});
